...
import com.itschool.library.exception.CustomerNotFoundException;
...
import com.itschool.library.respositories.BorrowRecordRepository;
...
public class BorrowRecordServiceImpl implements BorrowRecordService {
    ...
    public BorrowRecordServiceImpl(BookRepository bookRepository, CustomerRepository customerRepository, BorrowRecordRepository borrowRecordRepository) {
    ...
    public void borrowBook(BorrowRecordDTO borrowRecordDTO) {
        Book book = bookRepository.findById(borrowRecordDTO.getBookId()).orElseThrow(() -> new BookNotFoundException("Book not found"));

        if (book.getCopiesAvailable() <= 0) {
            throw new IllegalStateException("Not enough books");
        }

        Customer customer = customerRepository.findById(borrowRecordDTO.getCustomerId()).orElseThrow(() -> new CustomerNotFoundException("Customer not found"));
